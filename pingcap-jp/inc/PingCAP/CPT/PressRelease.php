...
abstract class PressRelease
{
	/**
	 * Return the default WP_Query for press release posts to be used in instances
	 * that do not have an existing query (ex: blocks and page templates)
	 *
	...
	public static function getDefaultWPQuery(array $add_params = []): WP_Query
	{
		...
		$params = self::modifyQueryWithFilters($params, [
			'category' => sanitize_text_field(wp_unslash($_GET[Constants\QueryParams::PRESS_RELEASE_ARCHIVE_FILTER_CATEGORY] ?? '')),
			'tag' => sanitize_text_field(wp_unslash($_GET[Constants\QueryParams::PRESS_RELEASE_ARCHIVE_FILTER_TAG] ?? '')),
			'search' => sanitize_text_field(wp_unslash($_GET[Constants\QueryParams::PRESS_RELEASE_ARCHIVE_FILTER_SEARCH] ?? ''))
		]);
	...
	/**
	 * Modifies a posts query object with the press release archive filter parameters.
	...
	/**
	 * Return the featured press release post id
	 *
	 * @return int
	 */
	public static function getFeaturedId(): int
	{
		return ACF::get_field_int(
			Constants\ACF::PRESS_RELEASE_SETTINGS_BASE . '_featured_post',
			'option'
		);
	}
	...
	public static function getPostsPerPageCount(): int
	{
		...
		$override_enabled = ACF::get_field_int(
			Constants\ACF::PRESS_RELEASE_SETTINGS_BASE . '_override_posts_per_page',
			'option'
		);

		if ($override_enabled) {
			$value = ACF::get_field_int(
				Constants\ACF::PRESS_RELEASE_SETTINGS_BASE . '_custom_posts_per_page',
				'option'
			);
		}
	...
	/**
	 * Return the post date of the press release post since that is used as its "category" label
	 *
	...
	/**
	 * Return the default image ACF object as set under "Press Releases / Press Release Settings / Images / Default Featured Image"
	 *
	 * @return array
	 */
	public static function getDefaultCardImage(): array
	{
		$cache_key = 'press_release_default_featured_image';

		$default_image = StaticCache::get($cache_key);

		if (!is_array($default_image)) {
			$default_image = ACF::get_field_array(Constants\ACF::PRESS_RELEASE_SETTINGS_BASE . '_default_featured_image', 'option');
	...
	public static function getArchiveTitle(): string
	{
		return ACF::get_field_string(
			Constants\ACF::PRESS_RELEASE_SETTINGS_BASE . '_archive_title',
			'option'
		);
	}
}
