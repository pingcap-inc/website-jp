...
{
	/**
	 * Return the default WP_Query for partner posts to be used in instances that
	...
	public static function getDefaultWPQuery(array $add_params = []): WP_Query
	{
		...
		$params = self::modifyQueryWithFilters($params, [
			'category' => sanitize_text_field(wp_unslash($_GET[Constants\QueryParams::PARTNER_ARCHIVE_FILTER_CATEGORY] ?? '')),
			'tag' => sanitize_text_field(wp_unslash($_GET[Constants\QueryParams::PARTNER_ARCHIVE_FILTER_TAG] ?? '')),
			'search' => sanitize_text_field(wp_unslash($_GET[Constants\QueryParams::PARTNER_ARCHIVE_FILTER_SEARCH] ?? ''))
		]);
	...
	/**
	 * Modifies a posts query object with the partner archive filter parameters.
	...
	/**
	 * Return the featured partner post id
	 *
	 * @return int
	 */
	public static function getFeaturedId(): int
	{
		return ACF::get_field_int(
			Constants\ACF::PARTNER_SETTINGS_BASE . '_featured_post',
			'option'
		);
	}
	...
	public static function getPostsPerPageCount(): int
	{
		...
		$override_enabled = ACF::get_field_int(
			Constants\ACF::PARTNER_SETTINGS_BASE . '_override_posts_per_page',
			'option'
		);

		if ($override_enabled) {
			$value = ACF::get_field_int(
				Constants\ACF::PARTNER_SETTINGS_BASE . '_custom_posts_per_page',
				'option'
			);
		}
	...
	/**
	 * Get the first category label attached to a partner post
	 *
	...
	/**
	 * Return the default image ACF object as set under "Partner Posts / Partner Settings / Images / Default Featured Image"
	 *
	 * @return array
	 */
	public static function getDefaultCardImage(): array
	{
		$cache_key = 'partner_default_featured_image';

		$default_image = StaticCache::get($cache_key);

		if (!is_array($default_image)) {
			$default_image = ACF::get_field_array(Constants\ACF::PARTNER_SETTINGS_BASE . '_default_featured_image', 'option');
	...
	public static function getArchiveTitle(): string
	{
		return ACF::get_field_string(
			Constants\ACF::PARTNER_SETTINGS_BASE . '_archive_title',
			'option'
		);
	}
}
