...
abstract class Video
{
	...
	public static function getDefaultWPQuery(array $add_params = []): WP_Query
	{
		...
		$params = self::modifyQueryWithFilters($params, [
			'category' => sanitize_text_field(wp_unslash($_GET[Constants\QueryParams::VIDEO_ARCHIVE_FILTER_CATEGORY] ?? '')),
		]);
    ...
    public static function getFeaturedId(): int
    {
        return ACF::get_field_int(
            Constants\ACF::VIDEO_SETTINGS_BASE . '_featured_post',
            'option'
        );
    }
    ...
    public static function getPostsPerPageCount(): int
    {
        ...
        $override_enabled = ACF::get_field_int(
            Constants\ACF::VIDEO_SETTINGS_BASE . '_override_posts_per_page',
            'option'
        );

        if ($override_enabled) {
            $value = ACF::get_field_int(
                Constants\ACF::VIDEO_SETTINGS_BASE . '_custom_posts_per_page',
                'option'
            );
        }
    ...
    {
        $cache_key = 'video_default_featured_image';

        $default_image = StaticCache::get($cache_key);

        if (!is_array($default_image)) {
            $default_image = ACF::get_field_array(Constants\ACF::VIDEO_SETTINGS_BASE . '_default_featured_image', 'option');
    ...
    public static function getArchiveTitle(): string
    {
        return ACF::get_field_string(
            Constants\ACF::VIDEO_SETTINGS_BASE . '_archive_title',
            'option'
        );
    }
}
